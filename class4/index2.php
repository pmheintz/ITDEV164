  ...
  <body>
    ...
    <p>
      ...
      if (file_exists('../uploads/highscores.txt')) {
        $result = file('../uploads/highscores.txt');
        print_r($result);
      } else {
        print_r('file not found');
      }
      /*
        if ($highScores = fopen('../uploads/highscores.txt', 'r')) {
          echo '<span class="success">'.fgets($highScores).'</span><br>'/PHP_EOL;
          while (!feof($highScores)) {
            echo fgets($highScores).'<br>'.PHP_EOL;
          }
        } else {
          echo "No current high scores.";
        }
        fclose($highScores); */
      ?>
    </p>
    ...
    <!-- Form for user's name and guess -->
